...
namespace WPGraphQL\Extensions\LearnPress\Data\Loader;

use GraphQL\Deferred;
use GraphQL\Error\UserError;
use WPGraphQL\Data\Loader\AbstractDataLoader;
use WPGraphQL\Extensions\LearnPress\CoreSchema;
use WPGraphQL\Extensions\LearnPress\Model\Course;


class LP_PostTypeLoader extends AbstractDataLoader {
	/**
	 * Stores loaded CPTs.
	 *
	 * @var array
	...
	public static function resolve_model( $post_type, $id, $fatal = true ) {
		switch ( $post_type ) {
			...
			default:
				$model = apply_filters( 'graphql_learnpress_cpt_loader_model', null, $post_type );
				if ( ! empty( $model ) ) {
					return new $model( $id );
				}
				...
				throw new UserError( sprintf( __( 'No Model is register to the custom post-type "%s"', 'wp-graphql-learnpress' ), $post_type ) );
		}
	}
	...
	public function loadKeys( array $keys ) {
		...
		$lp_post_types = CoreSchema::lp_post_types(); // \WP_GraphQL_WooCommerce::get_post_types();
		/**
		...
		foreach ( $keys as $key ) {
			...
			if ( ! in_array( $post_type, $lp_post_types, true ) ) {
				/* translators: invalid post-type error message */
				throw new UserError( sprintf( __( '%s is not a valid WooCommerce post-type', 'wp-graphql-learnpress' ), $post_type ) );
			}
			...
			$load_dependencies = new Deferred(
				function () use ( $key, $post_type, $customer_id, $parent_id, $context ) {
					 ...
					 * Post Objects
					 */
					do_action( 'wp_graphql_learnpress_cpt_loader_load_dependencies', $this, $key, $post_type );
	...
	/**
	 * Callback for inject the PostObject dataloader with lp_post models.
	 *
